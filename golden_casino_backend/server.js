...
app.get("/api/auth/user-info", auth(), async (req, res) => {
  try {
    ...
    // 롤링 요구사항을 충족했으면 전액 환전 가능, 아니면 베팅한 만큼만 환전 가능
    let maxExchangeAmount;
    if (rollingWagered >= rollingRequirement) {
      // 롤링 요구사항 완전 충족: 전액 환전 가능
      maxExchangeAmount = user.balance;
    } else {
      // 롤링 요구사항 미충족: 베팅한 만큼만 환전 가능 (점진적 환전)
      maxExchangeAmount = Math.min(user.balance, rollingWagered);
    }
    ...
    res.json({
      ...
      rollingRequirement: rollingRequirement,
      maxExchangeAmount,
    });
  } catch (err) {
    res.status(500).json({ message: "서버 에러" });
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.json({
        message: `잔액이 조정되었습니다. 현재 잔액: ${user.balance}코인`,
        balance: user.balance,
  ...
  try {
    if (bet.result === "win") {
      if (bet.choice === "player") {
        ...
        return bet.amount; // 뱅커 승리: 베팅액 1배 수익 (총 2배 지급)
  ...
  socket.on("cancel_bet", async (data) => {
    ...
    try {
      ...
      socket.emit("bet_cancelled_success", {
        message: `마지막 베팅이 취소되었습니다. (${choice} ${amount.toLocaleString()}코인)`,
...
const gameSchema = new mongoose.Schema({
  ...
  stats: {
    player: {
      ...
      total: { type: Number, default: 0 }, // 코인 단위 총 베팅액
    },
    banker: {
      count: { type: Number, default: 0 },
      total: { type: Number, default: 0 }, // 코인 단위 총 베팅액
  ...
  totalBets: { type: Number, default: 0 }, // 코인 단위 총 베팅액
...
const exchangeRequestSchema = new mongoose.Schema({
  ...
  username: String,
  requestAmount: Number, // 코인 단위
  actualAmount: Number, // 코인 단위
  fee: Number, // 코인 단위
...
app.post("/api/exchange/request", auth(), async (req, res) => {
  const { amount } = req.body; // amount는 코인 단위
  ...
  try {
    ...
    if (!amount || amount < 10) {
      return res
        .status(400)
        .json({ message: "최소 10코인부터 환전 가능합니다." });
    }
    ...
    if (amount > maxExchangeAmount) {
      return res.status(400).json({
        message: `환전 가능 금액을 초과했습니다. 최대 환전 가능: ${maxExchangeAmount.toLocaleString()}코인`,
      });
    }

    // 환전 수수료 계산 (10% 수수료, 하우스에게 유리하게 올림)
...
app.post("/api/deposit/request", auth(), async (req, res) => {
  ...
  try {
    ...
    if (amount < 5) {
      return res
        .status(400)
        .json({ message: "최소 5코인부터 충전 가능합니다." });
    }
...
app.post("/api/transfer/send", auth(), async (req, res) => {
  ...
  try {
    if (!toUserId || !amount || amount < 1) {
      return res
        .status(400)
        .json({ message: "올바른 송금 정보를 입력해주세요. (최소 1코인)" });
    }
