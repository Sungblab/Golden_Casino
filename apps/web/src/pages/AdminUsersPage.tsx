...
import { adminResetPassword, setAdminUserApproval } from "../api";
...
function UserRow({ token, user, busy, onToggle }: { token: string; user: AdminUser; busy: boolean; onToggle: () => void }) {
  ...
  return <tr><td><strong>{user.username}</strong><small className="admin-subline">{user.role === "admin" ? "관리자" : new Date(user.createdAt).toLocaleDateString("ko-KR")}</small></td><td><span className={`admin-status ${user.approved ? "live-status" : "muted-status"}`}>{user.approved ? "이용 중" : "정지"}</span></td><td>{COIN(user.balance)}</td><td><span className={user.wageringRemaining > 0 ? "admin-rolling-left" : "admin-rolling-done"}>{user.wageringRemaining > 0 ? COIN(user.wageringRemaining) : "완료"}</span></td><td>{user.totalBets.toLocaleString()}회</td><td>{user.wins}승 {user.losses}패</td><td>{user.role === "admin" ? <span className="admin-protected">보호됨</span> : <div className="admin-user-actions"><button className="admin-small-button" disabled={busy} onClick={onToggle}>{busy ? "처리 중" : user.approved ? "이용 정지" : "승인"}</button><button className="admin-small-button" disabled={resetting} onClick={() => void resetPassword()}>{resetting ? "처리 중" : "비번 초기화"}</button>{tempPassword && <div className="admin-temp-password"><span>임시 비밀번호: <code>{tempPassword}</code></span><button type="button" className="admin-temp-password-close" onClick={() => setTempPassword(null)} aria-label="닫기">×</button></div>}</div>}</td></tr>;
}
