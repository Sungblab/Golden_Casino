...
authRouter.post("/login", async (req, res) => {
  ...
  const result = await pool.query<{ id: string; username: string; password_hash: string; role: "user" | "admin"; approved: boolean }>(
    "SELECT id,username,password_hash,role,approved FROM users WHERE username=$1",
...
authRouter.post("/refresh", async (req, res) => {
  ...
  try {
    ...
    const user = await client.query<{ id: string; username: string; role: "user" | "admin"; approved: boolean }>(
      "SELECT id,username,role,approved FROM users WHERE id=$1",
      [current.user_id],
    );
    ...
    const publicUser = { id: user.rows[0].id, username: user.rows[0].username, role: user.rows[0].role };
    return res.json({ token: createToken(publicUser), user: publicUser });
  } catch (error) {
